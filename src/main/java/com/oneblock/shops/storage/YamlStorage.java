...
/**
 * YAML-based shop storage using {@code shops.yml}.
 *
 * <p>ItemStack data is serialised to Base64 via {@link BukkitObjectOutputStream}
 * so that full NBT (including custom items) is preserved.</p>
 */
public class YamlStorage implements StorageProvider {
    ...
    public List<Shop> loadAll() {
        ...
        for (String key : section.getKeys(false)) {
            try {
                ...
                String itemBase64 = s.getString("item");
                ItemStack item = itemBase64 != null ? fromBase64(itemBase64) : null;
    ...
    public void save(Shop shop) {
        ...
        shopsConfig.set(path + ".island_id",
                shop.getIslandId() != null ? shop.getIslandId().toString() : null);
        shopsConfig.set(path + ".item",
                shop.getItem() != null ? toBase64(shop.getItem()) : null);
    ...
    public void saveAll(List<Shop> shops) {
        shopsConfig.set("shops", null); // clear existing
    ...
    public void shutdown() {
        // saveAll is called by ShopManager before this
        flushToDisk();
    }
